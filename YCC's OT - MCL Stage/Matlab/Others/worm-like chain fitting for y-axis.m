...
Xfc = 0.07089; %force constant in x direction(pN/nm)
ForceOffset =30 ;
DNALengthOffset =  900;
crshift=0.2201;
Vshift=-0.09 ;    %initial value
%Temperature = 298; %unit in K % Need to change mannually in fitting
%function.

%load data
name='5mW-y-fe-h400-f100s5';
%pathname=[name '.xlsx'];
 pathname=[name '.txt'];
%initialdata=xlsread(pathname);
 [x z y]=textread(pathname,'%f%f%f');  %x: stage position, y: x-QPD, z:y-QPD
% x=initialdata(:,1);
% y=initialdata(:,2);
...
[yd xv yv]=textread('y-scan-1-1.txt','%f%f%f');
...
% a=polyfit(xv2,yd2,7);
 ...
 c0=13.7;                           %guess center point
  ...
  %center point
  cr=x(cr_index)+crshift;
  yr=y(cr_index)+Vshift;
...
for i=1:length(Fx)
    ...
    kz(i)=1/3 * kx(i);   %force constant in z-axis
end
